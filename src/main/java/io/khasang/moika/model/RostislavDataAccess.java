...
public interface RostislavDataAccess {
     ...
     * @filters опциональный набор элементов фильтрации в виде карты "имя поля -> пара: оператор, значение".
     * Если задана непустая карта, то фильтры при меняются по принципу "И" и по условию "значение
     * поля равно переданному заначению фильтра"
     *
     * @return результирующая коллекция строк сущности БД, подходящих по условиям фильтра.
     * Каждая результирующая строка представлена в виде карты "имя столбца - значение ячейки"
     */
